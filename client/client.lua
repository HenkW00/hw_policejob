...
RegisterNetEvent('hw_policejob:tackled', function(targetId)
    getTackled(targetId)
end)

RegisterNetEvent('hw_policejob:tackle', function()
    tacklePlayer()
end)
...
AddEventHandler('hw_policejob:searchPlayer', function()
...
AddEventHandler('hw_policejob:jailPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('hw_policejob:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
        ...
        if quantity < 1 then
            TriggerEvent('hw_policejob:notify', Strings.invalid_amount, Strings.invalid_amount_desc, 'error')
        else
            TriggerEvent('hw_policejob:sendToJail', GetPlayerServerId(player), quantity)
        end
    end
end)

AddEventHandler('hw_policejob:spawnVehicle', function(data)
    inMenu = false
...
AddEventHandler('hw_policejob:handcuffPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('hw_policejob:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
...
RegisterNetEvent('hw_policejob:arrested', function(pdId)
    isBusy = true
    local escaped
    ...
    if Config.Debug then
        print('^0[^1DEBUG^0] ^5You succesfully escaped.. Now run!')
        print('^0[^1DEBUG^0] ^5Player: ^3' .. pdId .. '^5 escaped from handcuffs!^0')
    end
    if not escaped then
        handcuffed()
    end
...
RegisterNetEvent('hw_policejob:arrest', function()
    isBusy = true
    lib.requestAnimDict('mp_arrest_paired', 100)
...
RegisterNetEvent('hw_policejob:uncuffAnim', function(target)
...
RegisterNetEvent('hw_policejob:uncuff', function()
    uncuffed()
end)

RegisterNetEvent('hw_policejob:stopEscorting', function()
    if not escorting.active then return end
    escorting.active = nil
    escorting.target = nil
end)

AddEventHandler('hw_policejob:escortPlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 2.0 then
        TriggerEvent('hw_policejob:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
...
AddEventHandler('hw_policejob:lockpickVehicle', function()
    ...
    if not DoesEntityExist(vehicle) then
        ...
        if dist < 2.5 then
            ...
            if Config.Debug then
                print('^0[^1DEBUG^0] ^5Succesfully opened vehicle: ^3' .. vehicle)
            end
        else
...
AddEventHandler('hw_policejob:impoundVehicle', function()
    local vehicle = ESX.Game.GetClosestVehicle()
    if not DoesEntityExist(vehicle) then
        TriggerEvent('hw_policejob:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
        ...
        if dist < 2.5 then
            ...
            if Config.Debug then
                print('^0[^1DEBUG^0] ^5Succesfully impounded the following vehicle: ^3' .. vehicle)
            end
        else
            TriggerEvent('hw_policejob:notify', Strings.too_far, Strings.too_far_desc, 'error')
        end
...
AddEventHandler('hw_policejob:vehicleInfo', function()
    local vehicle = ESX.Game.GetClosestVehicle()
    if not DoesEntityExist(vehicle) then
        TriggerEvent('hw_policejob:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
        ...
        if dist < 3.5 then
            ...
            if Config.Debug then
                print('^0[^1DEBUG^0] ^5 Succesfully retreived information from the following vehicle: ^3' .. vehicle)
            end
        else
            TriggerEvent('hw_policejob:notify', Strings.too_far, Strings.too_far_desc, 'error')
        end
...
AddEventHandler('hw_policejob:openBossMenu', function()
    if not hasJob then return end
	TriggerEvent('esx_society:openBossMenu', hasJob, function(data, menu)
        if Config.Debug then
            print('^0[^1DEBUG^0] ^5 Succesfully opened the bossmenu!^0')
        end
		menu.close()
	end, {wash = false})
end)

RegisterNetEvent('hw_policejob:escortedPlayer', function(pdId)
    if isCuffed then
        escorted.active = not escorted.active
        escorted.pdId = pdId
        if Config.Debug then
            print('^0[^1DEBUG^0] ^5Started escorting player: ^3' .. pdId)
        end
    end
end)

RegisterNetEvent('hw_policejob:setEscort', function(targetId)
    if not hasJob then return end
    escorting.active = not escorting.active
    escorting.target = targetId
end)

RegisterNetEvent('hw_policejob:putInVehicle', function()
    if isCuffed then
        ...
        if IsAnyVehicleNearPoint(coords, 5.0) then
			...
			if DoesEntityExist(vehicle) then
				...
				if freeSeat then
                    ...
                    if Config.Debug then
                        print('^0[^1DEBUG^0] ^5Placed suspect in a vehicle!')
                    end
				end
			end
		end
...
RegisterNetEvent('hw_policejob:takeFromVehicle', function()
	if IsPedSittingInAnyVehicle(cache.ped) then
		local vehicle = GetVehiclePedIsIn(cache.ped, false)
		TaskLeaveVehicle(cache.ped, vehicle, 64)
        if Config.Debug then
            print('^0[^1DEBUG^0] ^5Removed suspect from a vehicle!')
        end
	end
end)

AddEventHandler('hw_policejob:inVehiclePlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 4.0 then
        TriggerEvent('hw_policejob:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
    else
        TriggerServerEvent('hw_policejob:inVehiclePlayer', GetPlayerServerId(player))
    end
end)

AddEventHandler('hw_policejob:outVehiclePlayer', function()
    if not hasJob then return end
    local player, dist = ESX.Game.GetClosestPlayer()
    if player == -1 or dist > 4.0 then
        TriggerEvent('hw_policejob:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
    else
        TriggerServerEvent('hw_policejob:outVehiclePlayer', GetPlayerServerId(player))
    end
end)

AddEventHandler('hw_policejob:vehicleInteractions', function()
    vehicleInteractionMenu(vehicle)
    if Config.Debug then
        print('^0[^1DEBUG^0] ^5Succesfully opened the vehicle interaction menu!')
    end
end)

AddEventHandler('hw_policejob:placeObjects', function()
    placeObjectsMenu(vehicle)
end)

AddEventHandler('hw_policejob:spawnProp', function(index)
    ...
    if Config.Debug then
        print('^0[^1DEBUG^0] ^5Placed the following prop: ^3' .. prop.model)
    end
end)

AddEventHandler('hw_policejob:licenseMenu', function(data)
    if not hasJob then return end
    openLicenseMenu(data)
end)

AddEventHandler('hw_policejob:purchaseArmoury', function(data)
    if not hasJob then return end
    local data = data
    data.quantity = 1
    ...
    local canPurchase = lib.callback.await('hw_policejob:canPurchase', 100, data)
    if canPurchase then
        TriggerEvent('hw_policejob:notify', Strings.success, Strings.successful_purchase_desc, 'success')
    else
        TriggerEvent('hw_policejob:notify', Strings.lacking_funds, Strings.lacking_funds_desc, 'error')
    end
end)

AddEventHandler('hw_policejob:checkId', function(targetId)
    if not hasJob then return end
...
AddEventHandler('hw_policejob:revokeLicense', function(data)
    TriggerServerEvent('esx_license:removeLicense', data.targetId, data.license)
    TriggerEvent('hw_policejob:notify', Strings.license_revoked, Strings.license_revoked_desc, 'success')
    Wait(420) 
    checkPlayerId(data.targetId)
end)

AddEventHandler('hw_policejob:manageId', function(data)
    manageId(data)
end)
        ...
        if v.bossMenu.enabled then
            if v.bossMenu?.target?.enabled then
                local data = {
                    ...
                    options = {
                        {
                            event = 'hw_policejob:openBossMenu',
                            icon = 'fa-solid fa-suitcase-medical',
                            label = v.bossMenu.target.label
                        }
                    }
                }
                TriggerEvent('hw_policejob:addTarget', data)
            else
                CreateThread(function()
                    local textUI
                    while true do
...
if Config.handcuff.hotkey then
    RegisterCommand('cuffPlayer', function()
        TriggerEvent('hw_policejob:handcuffPlayer')
    end)
...
RegisterCommand('pdJobMenu', function()
    openJobMenu()
    if Config.Debug then
        print('^0[^1DEBUG^0] ^5Succesfully opened the interaction menu!')
    end
end)

AddEventHandler('hw_policejob:pdJobMenu', function()
    openJobMenu()
    if Config.Debug then
        print('^0[^1DEBUG^0] ^5Succesfully opened the interaction menu!')
    end
